...
def train_model(model: BaseEstimator, param_dict, data_dict, pp_dict=PP_DICT, savefig=None,
                save_model=True):
    ...
    m_err = compute_regression_result(pipeline, X_train, X_test, y_train, y_test, log_tf=log_tf)

    if save_model:
        store_pipeline(pipeline, param_dict, data_dict, pp_dict, m_err)

    return pipeline, m_err


def compute_regression_result(fitted_model, X_train, X_test, y_train, y_test, param_dict=None, log_tf=True):
    y_pred = fitted_model.predict(X_test)
    if log_tf:
        y_pred = DataManager.log_tf(y_pred)
...
def store_pipeline(pipeline, param_dict, data_dict, pp_dict, m_err=None, path=STORED_MODEL_PATH):
